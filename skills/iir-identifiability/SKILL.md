...
3. **Choose componentwise transformations `f` and `g`**
   - Default log-monomial case for positive or fixed-sign parameters:
     ```text
     θ* = f(θ) = log(θ)
     ψ(θ) = g(Af(θ)) = exp(A log(θ))
     ```
   - Treat this as the first practical class to try, especially for scale-based mechanistic models, not as the only possible IIR class.
   - If parameters may be negative but their signs are known or can be estimated, use the fixed-sign approach below.
   - If parameters are probabilities or otherwise bounded, consider an appropriate one-to-one componentwise transformation. Explain that sparse monomial labels are tied to the log-transformed case, while the invariant-subspace calculation itself can still be meaningful for other transformed coordinates.
   - Identity transformations may be useful in a sequential pass after an initial image reparameterisation, for example when testing additive reductions or linear combinations of monomial image coordinates.
   - Non-componentwise transformed coordinates can be used if the user has a scientifically meaningful invertible transformation, but then the agent should define `φ*` explicitly and avoid claiming monomial interpretation unless it is actually present.
...
6. **Run the existing IIR routines**
   ...
   - Interpret the result as minimal image, non-minimal image, or structurally identifiable but practically weakly informed.

7. **Choose interpretable image coordinates**
...
9. **Optional PWA / prediction analysis**
